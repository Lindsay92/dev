...
public class AuthServiceImpl implements AuthService {
    ...
    private final AuthHelper authHelper; // final on n'aura pas besoin de le
					 // modifier
	...
	Account account = Database.getByIdentifier(identifier);
	...
	if (account != null) {
		...
		String token = authHelper.createJWT(roles, name);
		...
		// envoi de la mess exceptin pour ne pas donner d'indices à
		// l'utilisateur
		// si on donne indice password wrong ou identifiant wrong, cela
		// l'aidera à pirater le compte
	    }
	} else {
	    throw new BadCredentialsException("Wrong credentials");
	    // BadCredentialsExpception fournis par spring
	}
    }
